        ...
        {!isMobile && (
          <header style={{ position: "absolute", top: 0, left: 0, right: 0, zIndex: 10, display: "flex", alignItems: "flex-start", padding: "36px 64px 36px 48px" }}>
            ...
            <nav style={{ flex: 1, display: "flex", justifyContent: "center", gap: "56px", marginLeft: "24px" }}>
              {navItems.map((item) => (
                <a key={item} href={item === "Collections" ? "/collections" : item === "Intimates" ? "/gallery" : "#"} style={{ color: "#fff", fontSize: "13px", letterSpacing: "0.1em", textDecoration: "none", fontFamily: "'Georgia', serif", fontWeight: 400, opacity: item === "Collections" ? 0.55 : 0.92 }}>{item}</a>
              ))}
            </nav>
